...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, n_heads):
        ...
        # All four are (d_model, d_model) -- the per-head split happens
        # via reshape AFTER this full-width projection, not before.
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # 2. Scaled dot-product attention, run independently per head
        #    via the leading (batch, n_heads) dimensions
        ...
        # 4. Final linear projection -- mixes information learned by
        #    different heads into a single coherent representation
        output = self.W_o(output)

        return output


# Demo usage -- sized to match Part 1's actual tiny German tokeniser
if __name__ == "__main__":
    ...
    # In the real pipeline this would be token_embedding(token_ids) * sqrt(d_model)
